...
year_equal_more, engine_volume_equal_more, price_equal_less = search_criteria

filtered_cars = []
for name, data in car_data.items():
    if data[1] >= year_equal_more and data[2] >= engine_volume_equal_more and data[4] <= price_equal_less:
        filtered_cars.append((name, data))

sorted_cars = sorted(filtered_cars, key=lambda x: x[1][4])

# Виводимо до 5 результатів
