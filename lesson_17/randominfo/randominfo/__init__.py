...
def full_path(filename: str) -> str:
    return abspath(join(dirname(__file__), 'images', filename))
...
def get_country(first_name = None):
    ...
    if first_name != None:
        ...
        country = choice(filteredData) if filteredData else "Unknown"
    return country
...
def get_alphabetic_profile_img(char, filePath, imgName, charColor = None, bgColor = None):
    ...
    if all((c in chars) for c in imgName):
        ...
        raise OSError("Invalid image name. Image name must contain valid characters.")
    return filePath
...
def get_address():
	...
	for i in range(5, 12):
		...
		if allAddrs:
			...
			full_addr.append("")  # дефолт, якщо даних немає
